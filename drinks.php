<?php
// DEBUG: tanggalin o i-comment out in production
// ini_set('display_errors',1); error_reporting(E_ALL);

include 'database.php'; // siguraduhin tama ang path at may $conn

// Kunin lahat ng Drinks mula sa database
...
<body>
  ...
  <h1 class="page-title">Refreshing Drinks</h1>

  <div class="grid-container">
    <?php while ($row = $result->fetch_assoc()): ?>
      <?php
        // safe output for attributes
        $pid = (int)$row['id'];
        $pname = htmlspecialchars($row['name'], ENT_QUOTES);
        $pprice = $row['price'];
        $pimg = !empty($row['image']) ? htmlspecialchars($row['image'], ENT_QUOTES) : 'img/drink_default.jpg';
      ?>
      <div class="product-card">
        <img src="<?= $pimg ?>" alt="<?= $pname ?>">
        <h2><?= $pname ?></h2>
        <p>₱<?= number_format($pprice, 2) ?></p>
        <!-- data attributes are safe now -->
...
<script>
  ...
  document.querySelectorAll('.buy-btn').forEach(btn => {
    console.log('found btn', btn.dataset.name);
    btn.addEventListener('click', function(){
      console.log('clicked buy', this.dataset);
